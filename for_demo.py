...
with detection_graph.as_default():
  with tf.Session(graph=detection_graph) as sess:
    while True:
      screen_ = np.array(ImageGrab.grab(bbox=(250,100,450,300)))
      screen_ = cv2.cvtColor(screen_, cv2.COLOR_BGR2RGB)
      traffic_light_red(screen_)
      image_3 = np.array(ImageGrab.grab(bbox=(0,40,800,600)))
      image_4 = process_img_3(image_3)
      image_5 = process_img_4(image_4)
      ...
      for i,b in enumerate(boxes[0]):
        ...
        if classes[0][i] == 3 or classes[0][i] == 6 or classes[0][i] == 8:
            if scores[0][i] >= 0.5:
              ...
              mid_x_ = (boxes[0][i][1]+boxes[0][i][3])/2
              mid_y_ = (boxes[0][i][0]+boxes[0][i][2])/2
      ...
      for i in range(0,len(a)):
        if(i==0):
          min_a = abs((b[1] - a[0])/b[1])*100
          min_x = abs((b[0] - x[0])/b[0])*100
          min_y = abs((b[2] - y[0])/b[2])*100
          min = min_x+min_y
          min_i = i
          
          
        else:
          min_a = abs((b[1] - a[i])/b[1])*100
          min_x = abs((b[0] - x[i])/b[0])*100
          min_y = abs((b[2] - y[i])/b[2])*100
          ch = min_x+min_y
          if(min > ch):
            min = ch
            min_i = i
      ...
      if x[0] != 0:
        ...
        min_x = abs((b[0] - x[min_i])/b[0])*100
        min_y = abs((b[2] - y[min_i])/b[2])*100
        min_a = abs((b[1] - a[min_i])/b[1])*100
              
        
        print(len(a)+1,"대 차가 식별됨, 그 중 대표 차 1대 추출")
        print("이전 object와의 오차율(거리) : ", min_a,"% 오차율")
        print("이전 object와의 오차율(x좌표) : ", min_x,"% 오차율")
        print("이전 object와의 오차율(y좌표) : ", min_y,"% 오차율")      
        ...
        print("내 차와의 거리(추정) : ", apx_distance)
        #cv2.putTextw(image_np, '{}'.forWmat(apx_distance), (int(mid_x*800),int(mid_y*450)), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255,255,255), 2)
        print("\n")
        if b[1] < a[min_i]:
        ...
        if(num==0):
          ...
          if(count==3 & control_last_num != 1):
              print("같은 입력이 3번 반복되었습니다. 차가 부딪흰 것으로 추정합니다.")
              control_car(4)
              count = 0
          else:
              control_car(control_num)
      ...
      print("\n")
      num_1 = 0
      b[0] = (boxes[0][min_i][1]+boxes[0][min_i][3])/2
      b[1] = round(((1 - (boxes[0][min_i][3] - boxes[0][min_i][1]))**4),1)
      b[2] = (boxes[0][min_i][0]+boxes[0][min_i][2])/2 
          
                    
      if cv2.waitKey(25) & 0xFF == ord('r'):
        print("차가 부딪혔습니다. 후진하겠습니다.")
        control_car(4)
        break            
      
      
      cv2.imshow('traffic_light',cv2.resize(screen_,(300,250)))
      cv2.imshow('object_detection_all',cv2.resize(image_np,(300,250)))
      cv2.imshow('grab_image',cv2.resize(image_3,(300,250)))
      cv2.imshow('ROI',cv2.resize(image_4,(300,250)))
      cv2.imshow('grayscale',cv2.resize(image_5,(300,250)))
      cv2.imshow('car_detection', cv2.resize(new_screen,(300,250)))
      cv2.imshow('street colar', cv2.resize(image1,(300,250)))
      cv2.imshow('object_detection_car',cv2.resize(image_np,(300,250)))
      cv2.imshow('object_detection_person',cv2.resize(image_np,(300,250)))
